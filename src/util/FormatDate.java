...
public class FormatDate extends Object {
    ...
    /**
    * Method for formating a Timestamp in default ("dd/MM/yyyy HH:mm:ss") format.
    */
    ...
    * @throws Exception
    ...
    public static java.sql.Timestamp parseTimestamp(
        String DATE, String PATTERN) throws Exception {
        java.sql.Timestamp date = null;
        if (
            (PATTERN != null) && (PATTERN.length() > 0)
                && !PATTERN.equalsIgnoreCase("null")) {
    ...
    * @throws Exception
    ...
    public static int subDate(java.util.Date BEFORE, java.util.Date AFTER) {
        long dif =
            (((((AFTER.getTime() - BEFORE.getTime()) / 1000) / 24) / 60) / 60);
    ...
    public static int subDateToday(java.util.Date DATE) {
        long dif =
            ((
                (
                    (
                        ((new java.util.Date()).getTime() - DATE.getTime()) / 1000
                    ) / 24
                ) / 60
            ) / 60);
    ...
    /*** Retorna data resultado da some dos dias com a data passada como parmetro*/
